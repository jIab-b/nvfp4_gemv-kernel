    ...
    uint16_t scales_b_packed)  // 2 FP8 E4M3 scales for B
{
    ...
    asm volatile(
        ...
        : "h"(scales_a_packed), "h"(scales_b_packed), "r"(a0), "r"(b0), "r"(a1), "r"(b1), "r"(a2), "r"(b2), "r"(a3), "r"(b3)
    )
    return result;
}
    ...
    bool valid)
{
    ...
    if (valid) {
        ...
        :
        : "r"(smem_a0), "l"(ptr_A + k_byte_offset + threadIdx.x * 32), "r"(smem_a1), "l"(ptr_A + k_byte_offset + threadIdx.x * 32 + 16)
    )
    } else {
        asm volatile(
    ...
    if (load_b) {
        ...
        "cp.async.cg.shared.global [%0], [%1], 16;\ncp.async.cg.shared.global [%2], [%3], 16;"
        :
        : "r"(smem_b0), "l"(ptr_B + k_byte_offset + threadIdx.x * 32), "r"(smem_b1), "l"(ptr_B + k_byte_offset + threadIdx.x * 32 + 16)
    )
        } else {
            asm volatile(
        "cp.async.cg.shared.global [%0], [%1], 16, 0;\ncp.async.cg.shared.global [%2], [%3], 16, 0;"
